...
double compute(double seat, double population){
    return population / (sqrt(seat * (seat + 1)));
}

Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    if(numSeats < populations.size()){
        error("can't meet the criteria");
    }
    Map<string, int> res;
    HeapPQueue states;
    for(auto x : populations){
        states.enqueue({x, 1 / (populations[x] / sqrt(2))});//intialize the priority queue
        res[x] = 1;//initialize the res
        numSeats--;
    }
    while(numSeats > 0){
        DataPoint highestState = states.dequeue();
        res[highestState.name] += 1;
        double newPopulation = 1 / compute(res[highestState.name], populations[highestState.name]);
                states.enqueue({highestState.name, newPopulation});
                numSeats--;
    }
    return res;
}
