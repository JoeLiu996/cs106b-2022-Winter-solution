...
void HeapPQueue::enqueue(const DataPoint& data) {
    if(logicalSize == allocatedSize - 1){
        allocatedSize *= 2;
        DataPoint* newArray = new DataPoint[allocatedSize];
    ...
    logicalSize++;
    int curIndex = logicalSize, parentIndex = curIndex / 2;
    while(parentIndex > 0 && elems[curIndex].weight < elems[parentIndex].weight){
        swap(elems[curIndex], elems[parentIndex]);
...
DataPoint HeapPQueue::peek() const {
    if(isEmpty()){
        error("the priority queue can't be empty");
    }
    return elems[1];
}
...
DataPoint HeapPQueue::dequeue() {
    ...
    swap(elems[logicalSize], elems[1]);
    logicalSize--;
    int curIndex = 1;
    //child and current node both need to be in the heap
    while(curIndex <= logicalSize / 2 && curIndex < logicalSize){
        int childIndex1 = curIndex * 2, childIndex2 = childIndex1 + 1;
        int childMin = childIndex1;
        if(childIndex2 <= logicalSize && elems[childIndex1].weight > elems[childIndex2].weight){
            childMin = childIndex2;
        }
        if(elems[curIndex].weight < elems[childMin].weight){
            break;
        }
        swap(elems[curIndex], elems[childMin]);
        curIndex = childMin;
    }
