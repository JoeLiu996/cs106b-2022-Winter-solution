...
//this function detect whether selected shift is overlap with shifts have been chosen
bool overlap(Shift select, const Set<Shift>& soFar){
    if(soFar.isEmpty()){
        return false;
    }
    else{
        for(Shift x : soFar){
          if(overlapsWith(select, x)){
            return true;
        }
    }
    return false;
...
//abstraction
int highestValueScheduleForHelper(int hoursRemaining, Set<Shift>& shifts,
                                         int curValue,
                                         Set<Shift>& res){
    int maxValue = 0;
    //base case 1
    ...
    //recursive case
        ...
        //choose
        Shift select = shifts.first();
        if(lengthOf(select) <= hoursRemaining && !overlap(select, res)){
        //explore
            //exclude
            Set<Shift> shiftWithout = res;//must be equal to res to keep track
            shifts.remove(select);
            int valueWithout = highestValueScheduleForHelper(hoursRemaining, shifts,
                                                             curValue, shiftWithout);
            //include
            Set<Shift> shiftWith = res;//must be equalt to res to keep track
            shiftWith.add(select);
            int valueWith = highestValueScheduleForHelper(hoursRemaining - lengthOf(select), shifts,
                                                          curValue + valueOf(select), shiftWith);
            //unchoose
            shifts.add(select);
            //compare
            maxValue = max(valueWith, valueWithout);
            if(maxValue == valueWith){
                res = shiftWith;
            }
            else {
                res = shiftWithout;
            ...
            shifts.remove(select);
            maxValue = highestValueScheduleForHelper(hoursRemaining, shifts, curValue, res);
            shifts.add(select);//IMPORTANT: ADD IT BACK!!!
                return maxValue;
        }
        }
}

/*This function will generate the shift schedule which will produce max value in restricted hours*/
Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    Set<Shift> copyShifts = shifts, res;
    if(maxHours < 0){
       error("restricted hours can't be negative");
    }else{
    highestValueScheduleForHelper(maxHours, copyShifts, 0, res);
    return res;
  }
}
