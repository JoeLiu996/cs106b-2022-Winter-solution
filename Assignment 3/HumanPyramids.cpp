...
double weightOnBackOfHelper(int row, int col, int pyramidHeight,
    ...
    double carryWeight;
    //edge case
    if(row < 0 || col < 0 || row < col || row > pyramidHeight - 1){
        error("the point is not in bounds");
    }
    //base case
    else if (row == 0){
        carryWeight = 0;
    ...
    //recursive case 1
        ...
        carryWeight = 80.0 + (weightOnBackOfHelper(row - 1, 0, pyramidHeight, table)) / 2;
        table[{row, col}] = carryWeight;
        return carryWeight;
    }
    //recursive case 2
    else{
        carryWeight = 160.0 + (weightOnBackOfHelper(row - 1, col - 1, pyramidHeight, table) +
                weightOnBackOfHelper(row - 1, col, pyramidHeight, table)) / 2;
        table[{row, col}] = carryWeight;
        return carryWeight;
    }
}
