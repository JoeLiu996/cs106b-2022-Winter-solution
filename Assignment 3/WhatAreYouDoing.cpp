...
void allEmphaseOfHelper(Set<string>& res, Vector<string>& tokens, string soFar){
    //base case
    if(tokens.isEmpty()){
        res.add(soFar);
    }
    //recursive case
            ...
            //choose
            string select = tokens[0];
            tokens.remove(0);
            if(isalpha(select[0])){
                //explore(two options)
                allEmphaseOfHelper(res, tokens, soFar + toUpperCase(select));
                allEmphaseOfHelper(res, tokens, soFar + toLowerCase(select));
  }
            else{
                allEmphaseOfHelper(res, tokens, soFar + select);
            }
            //unchoose
            tokens.insert(0, select);
    }
}
