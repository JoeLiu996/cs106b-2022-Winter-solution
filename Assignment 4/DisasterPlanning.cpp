...
#include "set.h"
using namespace std;
//havn't used numCities as constraints
...
bool canBeMadeDisasterReadyHelper(const Map<string, Set<string>>& roadNetwork,
    ...
    //edge case
    if(numCities < 0){
        error("max cities allowed can't be negative");
    }
    //base case 1
    ...
    //recursive case
        ...
        if(remaining.contains(city)){
            //explore
            //supply by stockpilling in the city
            Set<string> tempWith = supplyLocations + city;
            if(canBeMadeDisasterReadyHelper(roadNetwork, numCities - 1,
                                            tempWith, remaining - city - roadNetwork[city])){
                supplyLocations = tempWith;
                return true;
            }
            //supply by stockpilling in the neighbors
            for(auto neighbor : roadNetwork[city]){
                Set<string> tempWithout = supplyLocations + neighbor;
                if(canBeMadeDisasterReadyHelper(roadNetwork, numCities - 1,
                                                tempWithout, remaining - roadNetwork[neighbor] - neighbor)){
                    supplyLocations = tempWithout;
                    return true;
                }
            }
        }
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
  ...
  for(auto x : roadNetwork){
      remaining.add(x);
  }
  return canBeMadeDisasterReadyHelper(roadNetwork, numCities, supplyLocations, remaining);
}
