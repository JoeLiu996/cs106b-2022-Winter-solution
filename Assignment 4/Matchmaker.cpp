...
//still cant't pass small stress test!
...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    ...
    if(remaining.isEmpty()){
        ...
        //choose
        string node1 = remaining.firstKey();
        remaining.remove(node1);
        Set<string> corresponding = possibleLinks.get(node1);
        for(string node2 : corresponding){
            //avoid overlap computation
            if(remaining.containsKey(node2)){
            remaining.remove(node2);
            Pair partner(node1, node2);
            //temporily save soFar(can't start build now!)
            Set<Pair> temp = matching + partner;
            //explore
            if(hasPerfectMatching(remaining, temp)){
                //if has perfect matching, then start to build matching groups
                matching = temp;
                return true;
            }
            //unchoose
            remaining.put(node2, possibleLinks[node2]);
        }
      }
   }
    return false;
...
bool isPairedOff(const Pair &pair, const Set<Pair> &res) {
    bool paired = false;
    for (const Pair &link : res) {
...
int maximumWeightMatchingHelper(const Map<string, Map<string, int>>& possibleLinks,
                                Set<Pair>& res, int curValue, Set<string>& remaining){
    int maxTotalValue, maxTempValue = 0;
    //base case
    if(remaining.size() < 2){
        return curValue;
    }
    //recursive case
        ...
        //choose
        string node1 = remaining.first();
        remaining.remove(node1);
        //explore
        //exclude option 1
        Set<Pair> pairWithout = res;
        int valueWithout = maximumWeightMatchingHelper(possibleLinks, pairWithout, curValue, remaining);
        //unchoose
        remaining.add(node1);
        //include option 1(iterate option 2)
        int valueWith;
        Set<Pair> pairWith = res, pairWithFinal;
        for(auto node2 : possibleLinks[node1]){
            Pair nodes (node1, node2);
            if(!isPairedOff(nodes, pairWith)){
                remaining.remove(node1);
                remaining.remove(node2);
                pairWith.add(nodes);
                valueWith = maximumWeightMatchingHelper(possibleLinks, pairWith, curValue + possibleLinks[node1][node2], remaining);
                if(maxTempValue <= valueWith){
                    maxTempValue = valueWith;
                    pairWithFinal = pairWith;
                }
                //unchoose
                remaining.add(node2);
                pairWith.remove(nodes);
            }
        }
        //compare
        maxTotalValue = max(valueWithout, valueWith);
        if(maxTotalValue == valueWith){
            res = pairWithFinal;
        }
        else{
            res = pairWithout;
...
Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {
    ...
    for(auto x : possibleLinks){
        remaining.add(x);
    }
    Set<Pair> res = {};
    maximumWeightMatchingHelper(possibleLinks, res, 0, remaining);
    return res;
}
