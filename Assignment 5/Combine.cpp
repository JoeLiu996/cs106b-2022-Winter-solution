...
void split(const Vector<Vector<DataPoint>>& sequences, Vector<Vector<DataPoint>>& left, Vector<Vector<DataPoint>>& right){
    int n = sequences.size();
    left = sequences.subList(0, n / 2);
    right = sequences.subList(n / 2);
}
...
Vector<DataPoint> merge(const Vector<DataPoint>& left, const Vector<DataPoint>& right){
    ...
    while(idx1 < n1 && idx2 < n2){
        if(left[idx1].weight < right[idx2].weight){
            res.add(left[idx1]);
            idx1++;
        }
        else{
...
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    Vector<DataPoint> res, tempLeft, tempRight;
    ...
    split(sequences, left, right);
    tempLeft = combine(left);
    tempRight = combine(right);
    res = merge(tempLeft, tempRight);
    return res;
}
